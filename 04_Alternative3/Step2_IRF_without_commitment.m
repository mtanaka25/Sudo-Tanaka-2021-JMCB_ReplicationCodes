...
% A. Options
%--------------------------------------------------------------------
    SimCode     = 'Alternative3_IRF'        ; % Name of the mod file: simulation
...
%--------------------------------------------------------------------
% B-1.
    load(sprintf('%s\\Alternative3_est_original\\Alt3_original_draws.mat',  pwd));   
    nowLine = 1;
    ...
    for j = 1 : nDraws
        ...
        e2              = zeros(M_.exo_nbr ,LenIRFs);
        e2(nEpsQE,1)    = M_.Sigma_e(nEpsQE, nEpsQE)^(1/2);
        e2(nEpsGross,1) = M_.Sigma_e(nEpsQE, nEpsQE)^(1/2);
        for v = 2:LenIRFs
            e2(nEpsGross, v) = -(1 - rho_QE) * rho_QE^(v-2) * e2(nEpsGross,1);
        end
        
        irfj = zlbIRF_for_Alt3(M_, PHI1, PHI2, ... 
                                      'shocks'   , Shock2Plot,...
                                      'LengthIRF', LenIRFs,...
                                      'shockMat' , e2 );
