...
//Exogenous variables
  varexo eps_a_u eps_a_r eps_QE eps_QEs eps_z eps_zl eps_zeta eps_r eps_bL eps_g eps_mu
         eps_lmbdp eps_lmbdw eps_r1 eps_r2 eps_r3 eps_r4;
...
//Structual parameters
  parameters sigmau sigmar h nu omega tau1_p tau2_p tau3_p tau4_p tau5_p
             tau6_p psi eta Cratio zetap zetaw lmbdp lmbdw delta0 delta1;

//Policy parameters
  parameters rho_r rho_pi rho_dy;
...
estimated_params;
  // structual parameters
    sigmau  ,  gamma_pdf, 2.0000, 0.5000; // CCF (2012, EJ)
    sigmar  ,  gamma_pdf, 2.0000, 0.5000; // CCF (2012, EJ)
    ...
    eta     ,  gamma_pdf, 4.0000, 1.0000; // Smets and Wouters (2007, AER)
    zetap   ,   beta_pdf, 0.5000, 0.1000; // Chen et al.(2012, EJ)
    zetaw   ,   beta_pdf, 0.5000, 0.1000; // Chen et al.(2012, EJ)
    lmbdp   ,  gamma_pdf, 0.1500, 0.0200; // Justiniano et al.(2010)
    lmbdw   ,  gamma_pdf, 0.1500, 0.0200; // Justiniano et al.(2010)
    Cratio  ,  gamma_pdf, 1.0000, 0.0500; // CCF (2012, EJ)
    tau1_p  , normal_pdf, 1.5000, 0.5000; // CCF (2012, EJ)
    tau2_p  , normal_pdf, 1.5000, 0.5000; // CCF (2012, EJ)
    tau3_p  , normal_pdf, 1.5000, 0.5000; // CCF (2012, EJ)
    tau4_p  , normal_pdf, 1.5000, 0.5000; // CCF (2012, EJ)
    tau5_p  , normal_pdf, 1.5000, 0.5000; // CCF (2012, EJ)
    tau6_p  , normal_pdf, 1.5000, 0.5000; // CCF (2012, EJ)
    delta1  ,  gamma_pdf, 1.8200, 1.0000; // ALSN(2004, JMCB)
    delta0  ,  gamma_pdf, 4.3600, 1.0000; // ALSN(2004, JMCB)
  ...
  // steady state values
    gamma_p   , gamma_pdf, 1.3000, 0.2000; // Sample mean
    pi_ss_p   , gamma_pdf, 0.3000, 0.2000; // Sample mean
    zeta_ss_p , gamma_pdf, 1.0000, 0.2000; // Sample mean
  ...
  // size of shock
    ...
    stderr eps_r1   , inv_gamma_pdf, 0.10, inf; // same to eps_r
    stderr eps_r2   , inv_gamma_pdf, 0.10, inf; // same to eps_r
    stderr eps_r3   , inv_gamma_pdf, 0.10, inf; // same to eps_r
    stderr eps_r4   , inv_gamma_pdf, 0.10, inf; // same to eps_r
...
estimated_params_init;
    ...
    stderr eps_r4   , 0.012381610;
    stderr me_dw    , 1.149174402;
    stderr me_pi    , 0.237564980;
...
mode_compute=0, mode_file=Baseline_estimation_mode, mode_check,
mh_replic=200000, mh_nblocks=2,
mh_jscale=0.18, smoother
)y pi r rL RP RP_st RP_fl QE eQE mvCB mv;

//shock_decomposition(parameter_set=posterior_mean)rL_obs RP;