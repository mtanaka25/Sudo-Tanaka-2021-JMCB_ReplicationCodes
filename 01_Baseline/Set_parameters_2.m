...
% a-2. Estimated structual parameters 
    ...
    tau4    = x2(nowLine, 35)/1000;
    tau5    = x2(nowLine, 36)/1000;
    tau6    = x2(nowLine, 37)/1000;
    delta1  = x2(nowLine, 38);
    delta0  = x2(nowLine, 39);
    
% a-3. Estimated policy parameters
    rho_r     = x2(nowLine, 40);
    rho_pi    = x2(nowLine, 41);

% a-4. Estimated steady-state values
    gamma_p   = x2(nowLine, 42);
    pi_ss_p   = x2(nowLine, 43);
    zeta_ss_p = x2(nowLine, 44);
    
% a-5. Estimated shock parameters
    rho_bL    = x2(nowLine, 45);
    rho_QE    = x2(nowLine, 46);
    rho_a_u   = x2(nowLine, 47);
    rho_a_r   = x2(nowLine, 48);
    rho_z     = x2(nowLine, 49);
    rho_zeta  = x2(nowLine, 50);
    rho_g     = x2(nowLine, 51);
    rho_lmbdp = x2(nowLine, 52);
    rho_lmbdw = x2(nowLine, 53);
    rho_mu    = x2(nowLine, 54);
    rho_eQE   = x2(nowLine, 55);
